...
- Check for TypeScript errors by running `npx tsc -p electron/tsconfig.json` (or `npm run typecheck`) rather than building. **Never** invoke `tsc` standalone with bare `--module` / `--moduleResolution` / `--target` flags against a single file for a "quick check" — those flags don't reflect the project config (which is already on `node16`/`node16`) and the resulting warnings (most notoriously the `TS5107 moduleResolution=node10 is deprecated` notice) are artifacts of your own command line, not the codebase. If a single-file diagnostic is genuinely needed, add `-p electron/tsconfig.json` so the project config supplies the compiler options, or write a temporary scratch tsconfig that extends it. Treat any TS warning produced by a bespoke `tsc` invocation as suspect until reproduced through the project's configured entry points.
